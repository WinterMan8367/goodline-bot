...
analytics_bot = telebot.TeleBot("6690715932:AAFtlQFAFR61WzEhwFBfF1IjA0tp42XI6D4")
...
@analytics_bot.message_handler(commands=["graf"])
def send_photo_file(message):
  analytics_bot.send_photo(message.chat.id, 'https://i.imgur.com/ofwPfHE.png')
...
def start_keyboard(message):
  if user_dict.get(message.chat.id) != None:
    ...
    analytics_bot.send_message(message.chat.id, f"Какую информацию ты хочешь узнать? 💬", reply_markup = keyboard)
    
    user = user_dict[message.chat.id]
    msg_count = 2 if message.contact else 1
    if user.menu_id != None:
      analytics_bot.delete_message(message.chat.id, user.menu_id)
    user.menu_id = message.id + msg_count
    user.history.clear()
  else:
    analytics_bot.send_message(message.chat.id, "Сначала пройдите проверку, прежде чем использовать бота!")

# -----------------------------------------------------
  
@analytics_bot.message_handler(commands = ['debug'])
def debug(message):
  keyboard = types.InlineKeyboardMarkup()
  ...
  for item in user_dict:
    text += f"\n{item};"
  analytics_bot.send_message(message.chat.id, f"Текущие пользователи:\n{text}\n---------------------\nТекущий сеанс:\n\n{user_dict[message.chat.id]}", reply_markup = keyboard)

# Навигация в меню - [Обработки]
@analytics_bot.callback_query_handler(func = lambda call: True)
def test_callback(call):
  if call.message:
    ...
    if user_dict.get(chat_id) == None:
      analytics_bot.send_message(chat_id, "Авторизуйтесь по новой с помощью команды /start, прежде чем использовать бота!")
      analytics_bot.answer_callback_query(call.id)
      return

    user = user_dict[chat_id]

    if not user_verify(call.message, user.phone_number):
      analytics_bot.send_message(chat_id, f"Вы более не находитесь в списке сотрудников, доступ к боту прекращён.")
      analytics_bot.answer_callback_query(call.id)
      return
    ...
    if call.data == "reload":
      try:
        ...
        analytics_bot.edit_message_text(f"Текущие пользователи:\n{text}\n---------------------\nТекущий сеанс:\n\n{user}", chat_id, msg_id, reply_markup = keyboard)
        analytics_bot.answer_callback_query(call.id)
      except:
        analytics_bot.answer_callback_query(call.id)
    ...
    # -----------------------------------------------------
      ...
      if call.data != 'reload':
        analytics_bot.send_message(chat_id, "Error. Data: " + str(call.data))
        analytics_bot.answer_callback_query(call.id)
        return
    ...
    if call.data not in ['back', 'reload']:
      analytics_bot.edit_message_text(msg, chat_id, msg_id, reply_markup = keyboard)

    analytics_bot.answer_callback_query(call.id)

analytics_bot.infinity_polling()